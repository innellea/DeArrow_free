...
import {
    sendRealRequestToCustomServer,
    setupBackgroundRequestProxy,
} from "../maze-utils/src/background-request-proxy";
...
import {
    freeTrialActive,
    getContentScripts,
    getFreeAccessRequestTimeLeft,
    getFreeTrialTimeLeft,
    isActivated,
...
waitFor(() => Config.isReady())
    .then(() => {
        registerNeededContentScripts()
            .then(() => {
                if (!isFirefoxOrSafari()) {
                    ...
                    chrome.extension.isAllowedIncognitoAccess(
                        (isAllowedAccess) => {
                            if (
                                isAllowedAccess &&
                                !Config.config!.lastIncognitoStatus
                            ) {
                                registerNeededContentScripts(
                                    undefined,
                                    true
        ...
        if (!Config.config!.userID) {
            const setupUserID = async () => {
                ...
                if (!userID) {
                    const groupPolicyLicenseKey =
                        await getGroupPolicyLicenseKey();
                    ...
                    if (sponsorBlockConfig) {
                        ...
                        Config.config!.allowExpirements =
                            sponsorBlockConfig["allowExpirements"];
                        Config.config!.showDonationLink =
                            sponsorBlockConfig["showDonationLink"];
                        Config.config!.showUpsells =
                            sponsorBlockConfig["showUpsells"];
                        Config.config!.darkMode =
                            sponsorBlockConfig["darkMode"];
...
async function isPaywallEnabled(): Promise<boolean> {
    try {
        const result = await sendRealRequestToCustomServer(
            "GET",
            `${Config.config!.serverAddress}/api/featureFlag/deArrowPaywall`
        );
...
const existingRegistrations: {
    id: string;
    script: browser.contentScripts.RegisteredContentScript;
}[] = [];
...
chrome.runtime.onMessage.addListener((request, _, sendResponse) => {
    switch (request.message) {
        ...
        case "registerNeededContentScripts":
            registerNeededContentScripts(request.activated)
                .then(sendResponse)
                .catch(sendResponse);
            setupAlarms();
            return true;
...
async function registerNeededContentScripts(
    activated?: boolean,
    forceUpdate?: boolean
) {
    if (isSafari()) return;
    ...
    if (
        "scripting" in chrome &&
        "getRegisteredContentScripts" in chrome.scripting &&
        isPersistentContentScriptSupported()
    ) {
        Config.config!.firefoxOldContentScriptRegistration = false;

        const existingRegistration =
            await chromeP.scripting.getRegisteredContentScripts();
        if (existingRegistration?.length > 0) {
            const registrationsToRemove = existingRegistration.filter(
                (script) =>
                    forceUpdate ||
                    !contentScripts.some(
                        (newScript) => newScript.id === script.id
                    )
            );
        ...
        for (const script of contentScripts) {
            if (
                ...
                !existingRegistration.some(
                    (existing) => existing.id === script.id
                )
            ) {
        ...
        if (existingRegistrations.length > 0) {
            const registrationsToRemove = existingRegistrations.filter(
                (script) =>
                    forceUpdate ||
                    !contentScripts.some(
                        (newScript) => newScript.id === script.id
            ...
            for (const registration of registrationsToRemove) {
                ...
                existingRegistrations.splice(
                    existingRegistrations.indexOf(registration),
                    1
                );
            }
        }
...
function isPersistentContentScriptSupported() {
    ...
    const userAgentVersion = parseInt(
        navigator.userAgent.match(/Firefox\/(\S+)/)?.[1] ?? ""
    );
...
function setupAlarms() {
    if (!Config.config!.activated) {
        ...
        if (freeTrialTimeLeft && freeTrialTimeLeft > 0) {
            void chrome.alarms.create("freeTrial", {
                when: Date.now() + freeTrialTimeLeft + 1000 * 60,
            });
        ...
        if (freeAccessRequestTimeLeft && freeAccessRequestTimeLeft > 0) {
            void chrome.alarms.create("freeAccessRequest", {
                when: Date.now() + freeAccessRequestTimeLeft,
            });
